...
export default function RecurringTab() {
  ...
  // Cache system for recurring requests (max 100-200 requests, so caching is efficient)
  const cacheKey = 'paani_recurring_requests_cache';
  const cacheTimestampKey = 'paani_recurring_requests_cache_timestamp';
  const CACHE_DURATION = 60 * 60 * 1000; // 1 hour cache duration (cache auto-updates on create/update/delete)
  
  // Load from cache (memory + localStorage)
  const loadFromCache = (): RecurringRequest[] | null => {
    try {
  ...
  const saveToCache = (data: RecurringRequest[]) => {
    try {
  ...
  const refreshCache = async (): Promise<RecurringRequest[]> => {
    try {
  ...
  useEffect(() => {
    ...
    const fetchAll = async (forceRefresh = false) => {
      ...
      setError(null);
      try {
        // Fetch customers page 1 (paginated)
        const custRes = await fetch(buildApiUrl(`${API_ENDPOINTS.CUSTOMERS}?page=1&limit=50&sortBy=id&sortOrder=desc`));
        if (custRes.ok) {
          ...
          if (isActive) {
            ...
            setCustomerPage(1);
            setCustomerHasMore(c?.pagination?.hasNext ?? false);
          }
        }
        
        // Load recurring requests from cache or fetch
        let recurringData: RecurringRequest[] = [];
        // Try cache first (cache auto-updates on create/update/delete, so it's always fresh)
        const cached = loadFromCache();
        if (cached && cached.length > 0) {
          recurringData = cached;
          console.log('📦 Using cached recurring requests:', recurringData.length, '(cache auto-updates on changes)');
        } else {
          // Cache miss or expired, fetch from database
          recurringData = await refreshCache();
          console.log('🔄 Fetched recurring requests from database:', recurringData.length);
        }
        
        if (isActive) {
          setRecurringRequests(recurringData);
          // Attempt client-side due processing (non-blocking)
          try { triggerDueIfNeeded(recurringData); } catch {}
        }
      } catch (e) {
        // Fallback to cache on error
        const cached = loadFromCache();
        if (isActive) {
          setRecurringRequests(cached || []);
  ...
  const customersOptions = useMemo(() => {
    // PHASE 4: Ensure allCustomers is always an array
  ...
  const loadMoreCustomers = async () => {
    if (isLoadingMoreCustomers || !customerHasMore) return;
    setIsLoadingMoreCustomers(true);
    try {
      const nextPage = customerPage + 1;
      const res = await fetch(buildApiUrl(`${API_ENDPOINTS.CUSTOMERS}?page=${nextPage}&limit=50&sortBy=id&sortOrder=desc`));
      if (res.ok) {
        const c = await res.json();
        const newCustomers = Array.isArray(c) ? c : (c?.data || []);
        setAllCustomers((prev: Customer[]) => [...prev, ...newCustomers]);
        setCustomerPage(nextPage);
        setCustomerHasMore(c?.pagination?.hasNext ?? false);
      }
    } catch (e) {
      console.error('Error loading more customers:', e);
    } finally {
      ...
      if (isEditing && editingId) {
        ...
        if (res.ok) {
          ...
          setRecurringRequests(prev => {
            ...
            saveToCache(next); // Update cache with new entry
            return next;
          });
        } else {
            ...
            <div>
              ...
              <Select value={form.customerId} onValueChange={(v) => setForm(prev => ({ ...prev, customerId: v }))}>
                ...
                <SelectContent>
                  ...
                  {customerHasMore && (
                    <div
                      className="px-3 py-2 text-sm text-primary font-medium cursor-pointer hover:bg-accent border-t text-center"
                      onMouseDown={(e) => {
                        e.preventDefault();
                        e.stopPropagation();
                        loadMoreCustomers();
