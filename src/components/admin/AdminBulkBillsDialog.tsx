              ...
              <>
                ...
                {seqIndex !== null && seqBill ? (
                  <div className="rounded-xl border-2 border-primary/30 bg-primary/5 p-5 space-y-4">
                    ...
                    <div>
                      <p className="font-semibold text-base">{seqBill.name}</p>
                      <p className="text-xs text-muted-foreground mt-0.5">
                        ID: {seqBill.intId} &nbsp;·&nbsp; {seqBill.totalCans} cans &nbsp;·&nbsp; Rs. {seqBill.totalAmount.toFixed(0)}
                      </p>
                      {seqBill.phone && (
                        <p className="text-xs text-muted-foreground">📞 {seqBill.phone}</p>
                      )}
                      {!seqBill.phone && (
                        <p className="text-xs text-orange-500">No phone number — will download PDF only</p>
                      )}
                    </div>
                      ...
                      <table className="w-full text-xs">
                        <thead>
                          <tr className="bg-primary text-primary-foreground">
                            ...
                            <th className="px-2 py-2 text-right">Net Due</th>
                        ...
                        <tbody>
                          {bills.map((bill, i) => {
                            ...
                            const paidRow = bill.netPayable > 0 ? (bill.totalAmount + prevTotalRow) - bill.netPayable : 0;
                            const isCredit = bill.netPayable <= 0;
                            return (
                              <tr key={bill.intId} className={i % 2 === 0 ? '' : 'bg-muted/30'}>
                                <td className="px-2 py-1.5 text-center text-muted-foreground">{bill.intId}</td>
                                <td className="px-2 py-1.5">
                                  <p className="font-medium">{bill.name}</p>
                                  {bill.phone && <p className="text-muted-foreground text-[10px]">{bill.phone}</p>}
                                  {paidRow > 0 && <p className="text-green-600 text-[10px]">Paid Rs. {paidRow.toFixed(0)}</p>}
                                  {prevTotalRow > 0 && <p className="text-destructive text-[10px]">+Dues Rs. {prevTotalRow.toFixed(0)}</p>}
                                </td>
                                <td className="px-2 py-1.5 text-center">{bill.totalCans}</td>
                                <td className="px-2 py-1.5 text-right text-muted-foreground">Rs. {bill.totalAmount.toFixed(0)}</td>
                                <td className="px-2 py-1.5 text-right font-semibold">
                                  {isCredit
                                    ? <span className="text-green-600">CR Rs. {Math.abs(bill.netPayable).toFixed(0)}</span>
                                    : <span className="text-primary">Rs. {bill.netPayable.toFixed(0)}</span>
                                  }
                                </td>
                                <td className="px-2 py-1.5 text-center">
                                  <Button
                                    variant="ghost"
                                    size="sm"
                                    className="h-7 w-7 p-0 hover:bg-[#25D366]/10"
                        ...
                        <tfoot>
                          <tr className="bg-muted border-t-2 font-bold">
                            <td colSpan={2} className="px-2 py-2 text-xs">Total</td>
                            <td className="px-2 py-2 text-center text-xs">{totalCansAll}</td>
                            <td className="px-2 py-2 text-right text-xs text-muted-foreground">Rs. {totalAmountAll.toFixed(0)}</td>
                            <td className="px-2 py-2 text-right text-xs text-primary">Rs. {bills.reduce((s, b) => s + Math.max(0, b.netPayable), 0).toFixed(0)}</td>
                            <td />
                          </tr>
                        </tfoot>
                      </table>
