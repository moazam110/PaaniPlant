    ...
    if (editingRequest) {
      ...
      const fetchCustomersAndActiveRequests = async () => {
        ...
        try {
          ...
          const customersData = Array.isArray(customersResult) ? customersResult : (customersResult?.data || []);
          setAllCustomers(customersData);

          // Fetch active requests to prevent duplicates
          const requestsResponse = await fetch(buildApiUrl(`${API_ENDPOINTS.DELIVERY_REQUESTS}?page=1&limit=1000`));
          if (requestsResponse.ok) {
            const requestsResult = await requestsResponse.json();
            // PHASE 4: Handle paginated response - extract data array if paginated
            const requestsData: DeliveryRequest[] = Array.isArray(requestsResult) ? requestsResult : (requestsResult?.data || []);
